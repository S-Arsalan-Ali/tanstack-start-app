...
### 25. No Pagination on Admin Data Tables — Browser Performance Killer
...
As the store grows, this will severely impact the admin's browser:
- **Browser freezing/crashing** due to massive memory consumption from holding thousands of complex objects in React state.
- **Extreme load times** downloading megabytes of JSON data over the network.
- **Silent data loss** because Supabase silently truncates results at 1000 rows by default, meaning admins won't see older orders.

**Fix**:
- **Server-side Pagination**: Implement offset or cursor-based pagination (e.g., fetch 50 rows at a time).
- **Server-side Search/Filtering**: Move the `filtered = useMemo(...)` logic to the database query so the browser doesn't have to search through thousands of strings.
- **Data Virtualization**: For lists that must be long, use a library like `@tanstack/react-virtual` to only render the DOM nodes currently visible on screen.

---

### 26. Admin Role Check Duplicated Per-Page — Unnecessary Blocking
...
Each page fires its own `supabase.from("user_roles").select("role")` query. This causes:
- **Janky navigation**: Navigating between admin tabs feels slow because the browser pauses to re-verify the admin role on every single click.
- **Wasted Network Requests**: 6+ redundant database queries for data that hasn't changed.

**Fix**:
- **Shared Admin Context**: Create a `useAdmin()` hook that reads from a shared React Context established in the root `AdminGate`. Once verified, the browser remembers the admin status for the session.

---

### 32. Heavy Computations Run in Browser (Admin Panel)

**Files**: [admin.customers.lazy.tsx](file:///e:/lovable%20orange/chrome-chrome-main/ecommerce/src/routes/admin.customers.lazy.tsx#L39-L46)

The admin customers page calculates lifetime value (LTV) and order counts by downloading **every order** and looping through them in the browser:
```typescript
const stats: Record<string, { count: number; total: number }> = {};
(orders ?? []).forEach((o: any) => { ... stats calculation ... });
```

This will completely lock up the browser UI thread when order volume gets high.

**Fix**:
- Create a Supabase PostgreSQL View or RPC (e.g., `get_customer_stats()`) that calculates LTV at the database level. The database is highly optimized for this math; the browser is not.
...
## 🟠 PHASE 5 — Cloudflare Deployment & R2 Storage Migration (New)

### 33. Cloudflare R2 Storage Migration

**Files**: `src/routes/checkout.lazy.tsx`, `src/routes/admin.products.lazy.tsx`, `src/components/admin/ImageUpload.tsx`

Currently, images (receipts, products, store assets) are uploaded directly to Supabase Storage using `supabase.storage.from()`. The goal is to move these to Cloudflare R2 for cheaper egress and better edge performance.

**Note on Migration:** As requested, existing images in Supabase Storage will **not** be migrated. The system will start fresh with Cloudflare R2 for all new uploads.

**Fix**:
1.  **Provision R2 Bucket**: Create a bucket in Cloudflare (e.g., `motohelm-assets`).
2.  **Bind to Worker**: Add the R2 binding to `wrangler.jsonc`:
    ```jsonc
    "r2_buckets": [
      {
        "binding": "STORAGE",
    ...
    ```
3.  **Create Server API Endpoints**: Replace direct client-side uploads to Supabase with server functions (e.g., `/api/upload`) that use the Cloudflare environment binding (`env.STORAGE.put()`) to securely upload files to R2.
4.  **Public Access**: Enable public access (Custom Domain or `r2.dev` dev domain) on the R2 bucket to serve images directly via URL, replacing `getPublicUrl()`.

---

### 34. Cloudflare Workers Deployment Setup

The app is built with TanStack Start and already has the `@cloudflare/vite-plugin` configured in `vite.config.ts`. It's configured to deploy as a Cloudflare Worker via `wrangler.jsonc`.

**Step-by-Step Deployment Instructions**:

**Step 1: Install Wrangler CLI**
Run `npm install -g wrangler` and authenticate with `wrangler login`.

**Step 2: Configure Environment Variables (Secrets)**
Since we removed the Supabase Service Role Key from the `.env` file (Phase 1), you need to securely add your Supabase URL and Anon Key to Cloudflare.
Run:
```bash
wrangler secret put VITE_SUPABASE_URL
wrangler secret put VITE_SUPABASE_ANON_KEY
```
*(If you have a backend `SUPABASE_SERVICE_ROLE_KEY` needed for admin actions, put that in secrets as well).*

**Step 3: Update `wrangler.jsonc`**
Ensure your `wrangler.jsonc` matches your Cloudflare account requirements and includes the R2 binding mentioned above.

**Step 4: Build and Deploy**
Run the following commands to build the SSR output and deploy it to the Edge:
```bash
npm run build
wrangler deploy
```
...
In the Cloudflare Dashboard, go to Workers & Pages -> your worker -> Triggers -> Custom Domains, and attach your production domain (e.g., `motohelm.com`).
...
- **Cloudflare**: Verify images successfully upload to R2 and load on the storefront.
